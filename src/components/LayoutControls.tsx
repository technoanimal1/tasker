    ...
    <div className="flex gap-2">
      {LOGO_PRESETS.map((p) => {
        ...
        return (
          ...
          >
            <span
              className={`absolute h-2.5 w-3.5 -translate-x-1/2 -translate-y-1/2 rounded ${active ? 'bg-accent' : 'bg-white/[0.5]'}`}
...
export function AlignGrid({ value, onChange }: { value: Align9; onChange: (a: Align9) => void }) {
  return (
    <div className="grid grid-cols-3 gap-0.5 rounded-md bg-white/[0.06] p-0.5">
      {ALIGN9.map((a) => {
        ...
        return (
          <button
            key={a}
            onClick={() => onChange(a)}
            className={`grid h-6 w-6 place-items-center rounded transition ${active ? 'bg-accent' : 'hover:bg-white/[0.12]'}`}
          >
            <span className={`h-2 w-2 rounded ${active ? 'bg-panel' : 'bg-white/[0.4]'}`} />
          </button>
        )
      })}
    </div>
...
export function Slider({
  ...
  return (
    ...
    <div className="relative flex h-9 select-none items-center justify-between overflow-hidden rounded-lg bg-white/[0.06] px-3">
      ...
      <input
        ...
        className="relative z-30 w-14 cursor-text rounded bg-transparent px-1 text-right text-xs tabular-nums text-white outline-none focus:bg-panel focus:ring-1 focus:ring-accent"
      />
    </div>
  )
}
