    ...
    <div>
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            className="rounded-lg border border-zinc-700 bg-zinc-950 px-3 py-1.5 text-sm font-medium outline-none focus:border-brand"
          />
      ...
      <div className="grid gap-4 lg:grid-cols-[190px_1fr_260px]">
        ...
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wide text-zinc-500">Add layer</p>
          ...
          <div className="grid grid-cols-2 gap-2 pt-2">
            <label className="block">
              <span className="mb-1 block text-xs text-zinc-500">Canvas W</span>
              <input
                type="number"
                value={width}
                onChange={(e) => {
                ...
                className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-1 text-sm outline-none focus:border-brand"
              />
            </label>
            <label className="block">
              <span className="mb-1 block text-xs text-zinc-500">Canvas H</span>
              <input
                type="number"
                value={height}
                onChange={(e) => {
          ...
          <div className="pt-2">
            <label className="mb-1 block text-xs text-zinc-500">Canvas background</label>
            <input
              type="color"
              value={bg}
              onChange={(e) => {
              ...
              className="h-8 w-full cursor-pointer rounded border border-zinc-700 bg-zinc-950"
            />
          </div>
        </div>

        {/* Center: canvas */}
        <div className="overflow-auto rounded-xl border border-zinc-800 bg-zinc-950/60 p-6">
          <div
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="space-y-1">
              ...
              {layers.length === 0 && (
                <p className="px-2 py-4 text-center text-xs text-zinc-600">No layers yet</p>
              )}
            </div>
          </div>
...
function ToolButton({ label, hint, onClick }: { label: string; hint?: string; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="flex w-full items-center justify-between rounded-lg border border-zinc-800 bg-zinc-900/60 px-3 py-2 text-left text-sm hover:border-zinc-700 hover:bg-zinc-800"
    >
      <span>{label}</span>
    ...
    <div className="rounded-xl border border-zinc-800 bg-zinc-900/60 p-3">
        ...
        <div className="mt-3 space-y-2 border-t border-zinc-800 pt-3">
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-1.5 text-sm outline-none focus:border-brand"
            />
          </div>
          <div className="grid grid-cols-2 gap-2">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <div>
              <label className="mb-1 block text-[11px] text-zinc-500">Align</label>
              <select
                value={layer.align ?? 'left'}
                onChange={(e) => onChange({ align: e.target.value as Layer['align'] })}
                className="h-8 w-full rounded border border-zinc-700 bg-zinc-950 px-2 text-sm outline-none focus:border-brand"
              >
      ...
      {layer.type === 'rect' && (
        <div className="mt-3 grid grid-cols-2 gap-2 border-t border-zinc-800 pt-3">
          <div>
            <label className="mb-1 block text-[11px] text-zinc-500">Fill</label>
            <input
              type="color"
      ...
      {layer.type === 'image' && (
        <div className="mt-3 space-y-1 border-t border-zinc-800 pt-3">
          <label className="block text-[11px] text-zinc-500">Image URL</label>
          <input
            value={layer.src ?? ''}
