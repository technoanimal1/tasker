...
export function BranchMenu({ api, activeBranch, onSelect }: Props) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 rounded-pill border border-ring bg-panel/70 px-3 py-1.5 text-sm transition hover:border-white/[0.16]"
      >
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 z-20 mt-2 w-72 rounded-card border border-ring bg-[#111214] p-2 shadow-2xl">
            <div className="max-h-64 overflow-y-auto">
              {api.branches.map((b) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-pill px-3 py-2 text-left text-sm transition ${
              ...
              {mode === 'none' ? (
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-pill bg-white/[0.06] px-3 py-1.5 text-sm hover:bg-white/[0.12]"
                  >
                    + New
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-pill bg-white/[0.06] px-3 py-1.5 text-sm hover:bg-white/[0.12] disabled:opacity-50"
                  >
                    ⑂ Fork current
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    className="w-full rounded-pill border border-white/[0.16] bg-deep px-3 py-1.5 text-sm outline-none focus:border-accent"
                  />
                  <div className="flex gap-2">
                    <button
                      onClick={submit}
                      disabled={busy || !name.trim()}
                      className="flex-1 rounded-pill bg-white px-3 py-1.5 text-sm font-semibold text-black hover:bg-yellow disabled:opacity-60"
                    >
                    ...
                    <button
                      ...
                      className="rounded-pill border border-white/[0.16] px-3 py-1.5 text-sm"
                    >
                      Cancel
                    </button>
                  </div>
