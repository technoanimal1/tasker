...
export function BranchMenu({ api, activeBranch, onSelect }: Props) {
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        className="flex items-center gap-2 rounded-lg border border-slate-700 bg-slate-900 px-3 py-1.5 text-sm"
      >
        <span className="text-slate-500">⑂</span>
        <span className="max-w-[160px] truncate font-medium">{activeBranch?.name ?? 'branch'}</span>
        <span className="text-slate-500">▾</span>
      </button>
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 z-20 mt-2 w-72 rounded-xl border border-slate-800 bg-slate-900 p-2 shadow-2xl">
              ...
              {api.branches.map((b) => (
                <button
                  ...
                  className={`flex w-full items-center justify-between rounded-lg px-3 py-2 text-left text-sm hover:bg-slate-800 ${
                    activeBranch?.id === b.id ? 'bg-slate-800' : ''
                  }`}
                >
                  <span className="truncate">
                    {b.name}
                    {b.is_default && <span className="ml-2 text-xs text-slate-500">default</span>}
                  </span>
                  {activeBranch?.id === b.id && <span className="text-brand">✓</span>}
                </button>
              ))}
            </div>
              ...
              {mode === 'none' ? (
                <div className="flex gap-2">
                  <button
                    ...
                    className="flex-1 rounded-lg bg-slate-800 px-3 py-1.5 text-sm hover:bg-slate-700"
                  >
                    ＋ New
                  </button>
                  <button
                    ...
                    className="flex-1 rounded-lg bg-slate-800 px-3 py-1.5 text-sm hover:bg-slate-700 disabled:opacity-50"
                  >
                    ⑂ Fork current
                  </button>
                </div>
