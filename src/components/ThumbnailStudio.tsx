    ...
    <div className="flex flex-col gap-3 lg:h-[calc(100vh-7.5rem)]">
      ...
      <div className="flex flex-wrap items-center justify-between gap-2 px-1">
        <div className="flex min-w-0 flex-1 items-center gap-2">
          ...
          <span className="hidden shrink-0 truncate text-sm font-medium text-white sm:inline">{headerTitle}</span>
          {!singleView && (
            <div className="relative min-w-0 flex-1 sm:max-w-xs">
              <svg className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-dim" width="14" height="14" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      ...
      {!singleView && !mobilePanel && !providerPickerOpen && (
        ...
        >
          <button
            ...
            className="pointer-events-auto flex min-w-0 items-center gap-1.5 rounded-full border border-white/[0.16] bg-panel/95 px-4 py-2.5 text-xs font-semibold text-white shadow-xl backdrop-blur"
          >
            <span className="max-w-[38vw] truncate">{activeProvider ?? 'All providers'}</span>
            <ChevronDown size={14} className="text-dim" />
          </button>
          <div className="pointer-events-auto flex shrink-0 items-center gap-1 rounded-full border border-white/[0.16] bg-panel/95 p-1 text-xs shadow-xl backdrop-blur">
            {(['color', 'white'] as const).map((v) => (
              <button
                ...
                className={`rounded-full px-4 py-2 font-semibold transition ${
                  p.logoVariant === v ? 'bg-white text-black shadow' : 'text-muted'
                }`}
              >
          ...
          <div className="fixed inset-x-0 bottom-0 z-40 flex h-[70vh] flex-col rounded-t-[28px] border-t border-white/[0.16] bg-panel shadow-2xl lg:hidden">
            ...
            <div className="flex-1 space-y-1 overflow-y-auto p-2">
              ...
              >
                ...
                <span className="rounded-full bg-white/[0.06] px-2 py-0.5 text-[11px] text-muted">{totalCatalog}</span>
              </button>
              {providerCounts.map(({ provider, count }) => (
                ...
                >
                  ...
                  <span className="ml-2 shrink-0 rounded-full bg-white/[0.06] px-2 py-0.5 text-[11px] text-muted">{count}</span>
                </button>
              ))}
            </div>
          </div>
      ...
      <aside className={leftSheet}>
        <div className="flex items-center justify-between border-b border-ring px-3 py-2.5">
        ...
        <div className="border-b border-ring p-2">
          <div className="relative">
            <svg className="pointer-events-none absolute left-2.5 top-1/2 -translate-y-1/2 text-dim" width="13" height="13" viewBox="0 0 24 24" fill="none" aria-hidden="true">
              <circle cx="11" cy="11" r="7" stroke="currentColor" strokeWidth="2" />
              <path d="m20 20-3.5-3.5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" />
            </svg>
            <input
        ...
        <div className="flex-1 space-y-1.5 overflow-y-auto p-2">
          {!sidebarQuery && (
            ...
            >
              <span>All providers</span>
              <span className="rounded-full bg-white/[0.06] px-1.5 py-0.5 text-[10px] font-medium text-muted">{totalCatalog}</span>
            </button>
          )}
          {providerCounts.map(({ provider, count }) => {
            ...
            return (
              <div key={provider}>
                ...
                >
                  ...
                  <span className="rounded-full bg-white/[0.06] px-1.5 py-0.5 text-[10px] font-medium text-muted">
                ...
                {open && (
                  <div className="mt-0.5 space-y-0.5 pl-1">
                    {providerLoading === provider && loaded.length === 0 && (
                      <p className="flex items-center gap-1.5 px-2 py-2 text-[11px] text-dim">
                        ...
                        <div key={t.id} className="group/item relative">
                          ...
                          >
                            ...
                            <div className="min-w-0 flex-1 pr-6">
                              ...
                              <p className="text-[10px] text-dim">{hasOv ? 'custom' : t.provider}</p>
                            </div>
                          </button>
                          {isDesigner && (
                            <button
          ...
          >
              ...
              {gridItems.map((t) => {
                ...
                return (
                  <div
                    ...
                    className={`group relative flex cursor-pointer flex-col items-center gap-2 rounded-card p-2 transition ${
                      selectMode && picked
                        ...
                        : active
                          ? 'bg-white/[0.06] ring-1 ring-white/[0.28]'
                          : 'hover:bg-white/[0.04]'
                    }`}
                  ...
                  >
                    ...
                    <div className="w-full overflow-hidden rounded-card shadow-lg">
                      <GridTile
                        thumb={t}
                        params={pp}
                        assets={assetsFor(t)}
                    ...
                    {isDesigner && !selectMode && (
                      <button
                        ...
                        className="absolute left-3 top-3 z-10 grid h-7 w-7 place-items-center rounded-full bg-black/50 text-white opacity-0 shadow-md transition hover:bg-red-500/80 hover:text-white group-hover:opacity-100"
                      >
                        <Trash2 size={13} />
                      </button>
                    )}
                    <span className="max-w-full truncate text-[11px] text-muted">{t.name}</span>
                  </div>
                )
              })}
            </div>
            ...
            {gridItems.length === 0 &&
              (searching || allLoading || (activeProvider && providerLoading === activeProvider) ? (
                ...
                <p className="py-16 text-center text-sm text-dim">No games match “{gameSearch}”.</p>
              ) : (
                <p className="py-16 text-center text-sm text-dim">No games here yet.</p>
              ))}
            ...
            {!gameQuery && !activeProvider && (
              <div ref={loadMoreRef} className="mb-20 mt-5 flex items-center justify-center lg:mb-5">
                {allLoading ? (
                  ...
                  <span className="text-xs text-dim">That’s all {gridItems.length} thumbnails.</span>
                ) : null}
              </div>
            )}
          </div>
      ...
      <aside className={rightSheet} style={{ top: `${sheetTopVh}vh`, transition: sheetDragging ? 'none' : 'top 0.22s ease' }}>
        ...
        >
          <div className="h-1.5 w-10 rounded-full bg-white/[0.22]" />
        </div>
        <div className="flex items-center justify-between border-b border-ring px-3 pb-2.5 lg:hidden">
        ...
        {selected && selectedParams && (
          <div
            className="shrink-0 overflow-hidden border-b border-ring lg:hidden"
            style={{
              height: '23vh',
              backgroundImage: 'radial-gradient(circle at center, #1a1c22 1px, transparent 1px)',
              backgroundSize: '22px 22px',
        ...
        <div className="border-b border-ring p-3">
          {editingBranch ? (
            ...
            <p className="text-[11px] text-dim">Main template (read-only). Select your branch above to customise the frame design.</p>
          )}
        </div>
        ...
        <div className="flex-1 space-y-4 overflow-y-auto p-3">
          ...
          {showDesignerSections && (() => {
            ...
            return (
              <Section title={scope === 'selected' && selected ? `Colour · ${selected.name}` : 'Colour'}>
                ...
                {scope === 'selected' && (
                  <p className="text-[11px] text-dim">Overrides the auto colour for this thumbnail · saved instantly.</p>
                )}
              </Section>
            )
          })()}
          ...
          {showDesignerSections && (
            <Section title="Background">
              ...
              <p className="text-[11px] text-dim">Centered · fills the frame.</p>
            </Section>
          )}
              ...
              <>
                ...
                <Section title={`Logo · ${p.sizeKey}`}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    {hasLayout && (
                      <button onClick={resetLayout} className="text-[11px] text-muted underline hover:text-white">
                        Auto
                      </button>
                    )}
                  </div>
          ...
          {showFrameSections && (
            <Section title="Logo style">
              ...
              <Row label="Text logo">
                ...
                <Toggle
                  checked={p.textLogo}
                  onChange={(v) => (v ? setMany({ ...TEXT_LOGO_PRESET, textLogo: true }) : set('textLogo', false))}
                />
              </Row>
              {p.textLogo && (
                <>
                  ...
                  <Row label="Capital letters">
                    <Toggle checked={p.textAllCaps} onChange={(v) => set('textAllCaps', v)} />
                  </Row>
                  <Row label="Shadow">
                    <Toggle checked={p.textShadow} onChange={(v) => set('textShadow', v)} />
                  </Row>
                  <Row label="Vary line widths">
                    <Toggle checked={p.textFillLines} onChange={(v) => set('textFillLines', v)} />
                  </Row>
                </>
              )}
            </Section>
              ...
              {p.animEnabled && (
                <>
                  ...
                  {FX_SLOTS.map((slot) => {
                    ...
                    return (
                      <div key={slot} className="space-y-2 rounded-card border border-ring bg-panel p-2">
                        ...
                        <p className="text-[10px] uppercase tracking-wide text-dim">Effect</p>
                  ...
                  <p className="text-[11px] text-dim">Export with the “Animated (WebM)” format to save the motion.</p>
                </>
              )}
            </Section>
          )}
        ...
        {!lockedForClient && (
          <div className="flex gap-2 border-t border-ring p-3">
            <button
              onClick={handleSave}
...
function FitPreview({
  ...
  return (
    <div ref={ref} className="flex h-full w-full items-center justify-center p-3">
      {dw > 0 && (
        <div className="overflow-hidden rounded-card shadow-2xl">
